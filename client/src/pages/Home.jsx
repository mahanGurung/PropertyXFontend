    ...
    <>
            ...
            <div className="lg:col-span-6 xl:col-span-5">
              ...
              <div className="mt-8 flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                <Button 
                  ...
                  className="bg-primary hover:bg-primary-600 text-white px-6 py-6 rounded-lg text-base font-medium shadow-lg transition flex items-center justify-center"
                >
                  <i className="fas fa-coins mr-2"></i> Buy PXT Token
                </Button>
                <Link href="/explore"
                  className="bg-white text-secondary hover:bg-neutral-100 px-6 py-6 rounded-lg text-base font-medium shadow-lg transition flex items-center justify-center"
                >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-heading font-bold text-secondary">How PropertyX Benefits You</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {benefitsData.map((benefit, index) => (
            <div key={index} className="bg-white p-6 rounded-lg shadow-md">
              ...
              <h3 className="text-xl font-heading font-semibold text-secondary mb-3">{benefit.title}</h3>
      ...
      <div className="bg-neutral-50 py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-heading font-bold text-secondary">Tokenomics</h2>
            <p className="mt-4 text-lg text-neutral-300 max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {tokenomicsData.map((token, index) => (
              <div key={index} className={`bg-white p-6 rounded-lg shadow-md border-t-4 border-${token.color}`}>
                <h3 className="text-xl font-heading font-semibold text-secondary mb-3">{token.name}</h3>
                ...
                <ul className="space-y-2 text-neutral-300">
                  {token.features.map((feature, i) => (
                    <li key={i} className="flex items-start">
                      <i className={`fas fa-check-circle text-${token.color} mt-1 mr-2`}></i>
                      <span>{feature}</span>
                    </li>
                  ))}
                </ul>
                <div className="mt-6">
                  {token.button.action === 'buyPxt' ? (
                    <Button 
                      onClick={() => setBuyTokenModal(true)} 
                      className={`w-full bg-${token.color} hover:bg-${token.color}-600 text-white px-4 py-2 rounded-lg text-base font-medium transition`}
                    >
                      {token.button.text}
                    </Button>
                  ) : (
                    <Link 
                      href={token.button.action === 'exploreAssets' ? '/explore' : '/staking'}
                      className={`block text-center w-full bg-${token.color} hover:bg-${token.color}-600 text-white px-4 py-2 rounded-lg text-base font-medium transition`}
                    >
                      {token.button.text}
                    </Link>
                  )}
