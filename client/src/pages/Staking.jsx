  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-heading font-bold text-secondary">Stake & Vote</h1>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          <TabsList className="grid grid-cols-2 max-w-md mx-auto">
            <TabsTrigger 
              value="staking" 
              className={`py-2 px-4 ${activeTab === 'staking' ? 'bg-primary text-white' : 'bg-white text-neutral-400'} rounded-l-lg`}
            >
              Staking
            </TabsTrigger>
            <TabsTrigger 
              value="governance" 
              className={`py-2 px-4 ${activeTab === 'governance' ? 'bg-primary text-white' : 'bg-white text-neutral-400'} rounded-r-lg`}
            >
              Governance Voting
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="governance">
            <div className="bg-white rounded-lg shadow-md p-6 mb-8">
              <h3 className="text-xl font-heading font-semibold text-secondary mb-4">Asset Tokenization Proposals</h3>
              ...
              {!connected ? (
                <div className="text-center p-8">
                  ...
                  <h3 className="text-lg font-heading font-medium text-secondary mb-2">Connect Your Wallet</h3>
                  <p className="text-neutral-300 mb-6">Connect your Stacks wallet to view and vote on tokenization proposals.</p>
                ...
                <div>
                  <div className="flex justify-between items-center mb-6">
                    <p className="text-neutral-300 text-sm">You have <span className="font-semibold text-secondary">0 PXT</span> staked, giving you <span className="font-semibold text-primary">0 votes</span>.</p>
                    <p className="text-neutral-300 text-sm">Min. stake required: <span className="font-semibold text-secondary">100 PXT</span></p>
                  </div>
                      ...
                      return (
                        <div key={index} className="py-6 first:pt-0 last:pb-0">
                          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4">
                            <div>
                              <h4 className="text-lg font-heading font-semibold text-secondary">{proposal.assetName}</h4>
                              <p className="text-sm text-neutral-300 mb-2">{proposal.description}</p>
                              <div className="flex items-center mb-2 text-xs text-neutral-400">
                          ...
                          <div className="mb-4">
                            <div className="flex justify-between mb-1">
                              <span className="text-xs text-neutral-300">Current Vote</span>
                              <span className="text-xs font-medium">
                                Yes: {proposal.votes.yes.toLocaleString()} ({votePercent.yes}%) | 
                                No: {proposal.votes.no.toLocaleString()} ({votePercent.no}%)
                              </span>
                            </div>
                            <div className="w-full bg-neutral-100 rounded-full h-2 flex overflow-hidden">
                              <div className="bg-success h-2" style={{ width: `${votePercent.yes}%` }}></div>
                              <div className="bg-error-500 h-2" style={{ width: `${votePercent.no}%` }}></div>
                            </div>
                          </div>
                          
                          <div className="grid grid-cols-2 gap-4 text-xs">
                            <div>
                              <p className="text-neutral-300">Value: <span className="font-semibold text-secondary">${proposal.requestedValue.toLocaleString()}</span></p>
                            </div>
                            <div>
                              <p className="text-neutral-300">Voting Ends: <span className="font-semibold text-secondary">{proposal.voteEnds.toLocaleDateString()}</span></p>
                            </div>
                          </div>
                        </div>
                      );
        ...
        <Dialog open={showVoteModal} onOpenChange={setShowVoteModal}>
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              <DialogTitle>Vote on Asset Tokenization</DialogTitle>
              <DialogDescription>
            ...
            {selectedProposal && (
              <div className="py-4">
                <h3 className="text-lg font-heading font-semibold text-secondary mb-2">{selectedProposal.assetName}</h3>
                <p className="text-sm text-neutral-300 mb-4">{selectedProposal.description}</p>
                
                <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
                  <div>
                    <p className="text-neutral-300 mb-1">Asset ID</p>
                    <p className="font-medium">{selectedProposal.assetId}</p>
                  </div>
                  <div>
                    <p className="text-neutral-300 mb-1">Value</p>
                    <p className="font-medium">${selectedProposal.requestedValue.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-neutral-300 mb-1">Asset Owner</p>
                    <p className="font-mono font-medium">{shortenAddress(selectedProposal.assetOwner, 6)}</p>
                  </div>
                  <div>
                    <p className="text-neutral-300 mb-1">Voting Ends</p>
                    <p className="font-medium">{selectedProposal.voteEnds.toLocaleDateString()}</p>
                  </div>
                </div>
                
                <div className="mb-6">
                  <h4 className="font-medium text-secondary mb-3">Your Vote</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <Button
                      variant={userVote === true ? 'default' : 'outline'}
                      className={userVote === true ? 'bg-success hover:bg-success/90 text-white' : 'border-neutral-200'}
                    ...
                    <Button
                      ...
                      className={userVote === false ? 'bg-error-500 hover:bg-error-500/90 text-white' : 'border-neutral-200'}
                ...
                <div className="bg-neutral-50 p-4 rounded-md mb-4">
                  <p className="text-xs text-neutral-300">By voting, you are helping to determine whether this real-world asset should be tokenized on the PropertyX platform. Your vote is weighted by your staked PXT amount.</p>
                </div>
              </div>
            )}
            
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowVoteModal(false)}>Cancel</Button>
              <Button 
